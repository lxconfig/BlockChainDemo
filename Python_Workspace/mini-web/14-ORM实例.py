class ModelMetaclass(type):
    '''定义一个元类'''
    # name=User, bases=()表示User类没有父类
    # attrs={'uid': ('uid', 'int unsigned')...}表示User类的类属性,通常有很多属性
...
# 在创建类a时，遇到metaclass=xxx，就表示用xxx类来生成这个类a的类对象
class User(metaclass=ModelMetaclass):
    ...
    def __init__(self, **kwargs):
        for name, value in kwargs.items():
            ...
            # 格式是: uid=123, name='lx'...
            # 不用self.name设置的原因是 没法把name当作变量解析
    ...
    def save(self):
        ...
        fields, args = [], []
        # 在实例属性中找不到__mappings__，就回去类属性中找
