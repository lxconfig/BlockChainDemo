    ...
    def service_client(self, client_socket):
        # 接收客户端发送的请求数据
            ...
            if not file_name.endswith('.py'):
                # 请求静态资源
                try:
                    with open(self.static_path+file_name, "rb") as f:
                        file_content = f.read()
...
def main():
    """
        支持配置文件的服务器(由于没有static和dynamic文件夹 此代码不能执行)
        添加shell脚本
        1. 创建run.sh文件，写入 python3 xxxx.py 7890 WSGI_mini_frame:application
        2. 修改run.sh的权限  chmod +x run.sh
        3. 运行 ./run.sh
    """
    ...
    # sys.path.append(conf_info['frame_path'])
